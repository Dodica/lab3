...
Ship.prototype.draw = function (context) {
	context.translate(this.position.x, this.position.y);
	context.rotate(this.angle);
	var offset = KEY_STATUS.up && life > 0 ? this.width : 0; // Podešavanje offseta za animaciju broda.
	context.drawImage(
		this.texture,
		offset,
		0,
...
function animate() {
	// Poziva funkciju za animaciju na sljedećem frejmu
...
function handleTitlePage() {
	...
	if (KEY_STATUS.enter) {
		...
		// Postavlja naslovnicu na "false" i bilježi trenutno vrijeme
		titlePage = false;
		startTime = new Date();
	}
}
...
// Funkcija za crtanje vatrenih kugli (fireballs)
			...
			for (let x = asteroids.length - 1; x >= 0; x--) {
				if (fireball.intersects(asteroids[x])) {
					...
					if (asteroids[x].texture === imageRepo.asteroidBig) {
						...
						// Stvaranje četiri manja asteroida
						for (let a = 0; a < 4; a++) {
